 ...
 * the model class.
 */
template <class STATE> class StateVariableSet {

public:
  StateVariableSet()
      : maxGap(std::numeric_limits<VirtualTime>::max() / 2),
        hugeGap(std::numeric_limits<VirtualTime>::max() / 2), validityPeriod(1),
        historyLength(0), currentTime(std::numeric_limits<VirtualTime>::min()),
        enableInterpolation(false) {
#ifdef ENABLE_EXPERIMENTAL_FEATURES
    interpolate = boost::bind(&StateVariableSet::defaultInterpolate, this, _1,
                              _2, _3, _4, _5);
#endif
  }
  ...
  void setComputeFunction(
      const boost::function<const STATE(VirtualTime)> &callback) {
    compute = callback;
  }
  ...
  void setInterpolateFunction(
      const boost::function<STATE(const STATE &, const STATE &, VirtualTime,
  ...
  inline const STATE &getState(VirtualTime time,
                               bool forceNoInterpolation = false) {
#ifndef ENABLE_EXPERIMENTAL_FEATURES
    ...
    if (it == buffer.begin()) {
      ...
      s << "requested time = " << time
        << ", oldest history = " << buffer.begin()->first;
...
protected:
  ...
  inline void truncatePast() {
    ...
    if (buffer.size() > 1 &&
        buffer.begin()->first < (currentTime - historyLength)) {
  ...
  /// otherwise
  inline const STATE &getValueInterpolated(VirtualTime timeRequested,
                                           VirtualTime previousStep) {
#ifndef ENABLE_EXPERIMENTAL_FEATURES
    class ExperimentalFeatureNotEnabled {};
    throw ExperimentalFeatureNotEnabled();
#endif
    ...
    buffer[timeRequested] = interpolate(state1, state2, previousStep,
                                        previousStep + maxGap, timeRequested);
  ...
  boost::function<STATE(const STATE &, const STATE &, VirtualTime, VirtualTime,
                        VirtualTime)>
      interpolate;

  /// default interpolate function to throw an exception
  STATE defaultInterpolate(const STATE &, const STATE &, VirtualTime,
                           VirtualTime, VirtualTime) {
#ifndef ENABLE_EXPERIMENTAL_FEATURES
    class ExperimentalFeatureNotEnabled {};
    throw ExperimentalFeatureNotEnabled();
#endif
    std::cout << "Interpolation enabled but no interpolate function set."
              << std::endl;
    throw ChimeraTK::logic_error(
        "Interpolation enabled but no interpolate function set.");
  }
};
