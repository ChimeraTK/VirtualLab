...
/**
 *  A SignalSource provides time-dependent signal values to a SignalSink.
...
public:
  ...
  /** [call from backend/model] set callback function to be called when a new
   * value needs to be computed. The callback function must return the new
   * value. The new value will be automatically placed into the buffer.
   *
   *  It is mandatory to set this callback function!
   */
  void setCallback(const boost::function<double(VirtualTime)> &callback);

  /** [call from backend/model] set validity period. A value for the time T will
   * be used when a value for the time t < T+period is requested. Backends may
   * set this e.g. to the sampling time, since the output value will not change
   * during this time. Models may set this e.g. to a fraction of the model's
   * time constant to save computing time.
   *
   *  The validity period defaults to 1, which effectively disables the
   * functionality. The provided value must be larger than 0.
   */
  ...
  void setTimeTolerance(VirtualTime time) {
    std::cout << "SignalSource::setTimeTolerance(): This function is "
                 "DEPCRECATED, use setValidityPeriod() instead!"
              << std::endl;
    setValidityPeriod(time);
  }

  /** [call from backend/model] Set maximum time gap. If a value further into
   * the future of the latest computed value than the maximum gap time is
   * requested, intermediate values will be computed so that no values are
  ...
  /** [call from backend/model] Set - in addition to the maximum gap - an even
   * bigger gap, which will be used to fill larger steps. If a state is
   * requested into the far future (more than 2*hugeGap), intermediate steps
   *  with a distance of hugeGap (instead of maxGap) will be inserted to fill
   * the gap. Only the last hugeGap interval before the requested state will be
   * filled with intermediate steps in a distance of maxGap.
   *
  ...
  void setInterpolateFunction(
      const boost::function<double(const double &, const double &, VirtualTime,
                                   VirtualTime, VirtualTime)> &callback) {
   ...
   * setCallback()) to have a basis for the computations. Use getLatestTime() to
   * obtain the time of the latest state.
   *
  ...
  /** [call from model/backend] set callback function to be called when
   * setMaxHistoryLength() is called The callback's argument is the new history
   * length.
   */
  void
  setOnHistoryLengthChanged(const boost::function<void(VirtualTime)> &callback);
...
protected:
  ...
  inline double constantCallback(double) { return value; }
