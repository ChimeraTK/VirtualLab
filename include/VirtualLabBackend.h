/*
 * VirtualDevice.h - base class for MTCA.4u dummy devices that fit into the
 ...
 * Declare an event. Use instead of BOOST_MSM_EUML_EVENT, as we must not create
 ...
 * UserType is the data type the data should be accessed by. The conversion is
 ...
 * CONNECT_REGISTER_EVENT macro for write events. The table must be terminated
...
/** \def READEVENT_TABLE
 * Provide a "table" of events and register names using the
 * CONNECT_REGISTER_EVENT macro for read events The table must be terminated
 ...
 * Connect events with registers. The first argument eventName is the name of an
 * event previously declared using BOOST_MSM_EUML_EVENT. The second argument is
 * a DummyRegisterAccessor for the register to connect with. Use this macro in
 * the of the WRITE_EVENT_TABLE and READ_EVENT_TABLE macros. Do not separate
 * multiple calls to CONNECT_REGISTER_EVENT inside the same
 * WRITE_EVENT_TABLE/READ_EVENT_TABLE macro by any separator (i.e. no comma or
 ...
 * Declare a guard condition on the value of a register. The resulting guard
 * condition can be used on an event defined with the CONNECT_REGISTER_EVENT
 * macro. The first argument guardName is the name of the resulting guard class
 * and the second argument will be parsed as the guard condition (standard C++
 * syntax). Inside the condition, the variable "value" can be used which
 * contains the value of the register written in the event (type: int32_t,
 ...
 * Declare a guard condition. The argument guardName is the name of the
 * resulting guard class. The code must follow this macro, contain a return
 * statement returning a boolean and has to be terminated with
 * END_DECLARE_GUARD.
 *
 * (test coverage hint: these two macros are used in the test) */
 ...
 * Declare an action with arbitrary code. The argument actionName is the name of
 * the resulting action class. The code must follow this macro and has to be
 ...
 * Declare a timer group with a given name. The additional arguments must be the
 ...
 * Declare a state machine (usually a sub-state machine, also see
 * DECLARE_MAIN_STATE_MACHINE) dummyDeviceType is the class name of the
 * physDummyDevice implementation the state machine will be used in
 * stateMachineName is the name of the state machine itself. Several types will
 * be defined based on this names with one or more trailing underscores.
 * initialState is the name of the initial state. Multiple states can be added
 * by separating them with "<<". transitionTable is the transition table in eUML
 * syntax.
 *
 * (test coverage hint: this macro is used in the test) */
 ...
 * Declare the main state machine. This is just like DECLARE_STATE_MACHINE but
 * with a fixed name "mainStateMachine". Also the state machine will be
 * instantiated under the name "theStateMachine", as expected by other parts of
 * this framework.
 *
 * (test coverage hint: this macro is used in the test) */
 ...
 * Declare the constructor of the VirtualLabBackend. The first argument must be
 * the class name. The other arguments must be the list of member initialisers,
 * the code of the cunstructor follows this macro and must be terminated with
 * END_CONSTRUCTOR, even if no code is put into the constructor. This macro will
 * also insert the appropriate createInstance function used in the
 * BackendFactory. It will maintain a map of all instances, so the same instance
 * can be obtained multiple times when using the same instance name in the SDM
 * URI (like "instanceID" in
 * "sdm://./myVirtualLabBackend:instanceID=mapfile.map"). To register the
 * backend type with the BackendFactory, the macro REGISTER_BACKEND_TYPE must be
 * used.
 *
 * Sometimes you need to force the linkter to link the object code that
 * registers the backend in the factory (e.g. if you do not explicitly use any
 * other part of the library). In this case, just refer to
 * \verbatim<yourBackend>::backendRegisterer.dummy\endverbatim somewhere in your
 * code (e.g. by setting it to 1).
 *
 * @attention The backends are not thread-safe. When using a VirtualLabBackend
 * from a frontend Device concurrently in multiple threads, a transparent
 * locking decorator must be used. When using it concurrently from the
 * backend-side (e.g. through the backend register accessors, timers, sinks and
 ...
 * Initialise a sub-state machine. This should be called inside of the
 * constructor.
 *
 * (test coverage hint: this macro is used in the test) */
 ...
 * source file for any VirtualLabBackend even when not intending to use the
